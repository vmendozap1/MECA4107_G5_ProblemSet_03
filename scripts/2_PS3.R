...
testf <- mutate_if(testf, is.character, as.factor)
#normalizamos
trainf <- trainf %>%
...
ctrl<- trainControl(method = "cv",
                    number = 5,
                    index = block_folds$splits,
                    verboseIter = TRUE)

#Definimos la grilla de rpofundidad
mtry_grid<-expand.grid(mtry =c(60,70,80), # c(30,40,50!)
                       min.node.size= c(10,15,20), #c(30!,40,50) controla la complejidad del arbol
